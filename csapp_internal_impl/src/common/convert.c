...
/**
 * @brief parse string to uint by DFA
 *
 * [0] -- (cur == '0')   -> [1]  -- (cur == 'x')   -> [4] -- (cur == '0-9' | 'a-f') -> [5] -- (cur == '0-9' | 'a-f') -> [5]
 *                                                                                         -- (cur == ' ') -> [6]
 *
 *                               -- (cur == '0-9') -> [2]
 *     -- (cur == '1-9') -> [2]  -- (cur == '0-9') -> [2] -- (cur == ' ') -> [6]
 *     -- (cur == '-')   -> [3]  -- (cur == '0')   -> [1]
 *                               -- (cur == '1-9') -> [2]
 *     -- (cur == ' ')   -> [0]
 * @param str
...
{
    end = (end < 0) ? strlen(str) - abs(end) : end;
    uint64_t uv = 0;
    int sign_bit = 0;
    ...
    for (int i = start; i <= end; ++i)
    {
        ...
        else if (state == 2)
        {
            if ('0' <= c && c <= '9')
            {
                ...
                if (pv > uv)
                {
                    printf("(uint64_t)%s overflow: connot convert\n", str);
                    goto fail;
                }
                continue;
            }
