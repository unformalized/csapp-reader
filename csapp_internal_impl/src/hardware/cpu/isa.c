...
typedef enum OPERAND_TYPE
{
    ...
    MEM_IMM_REG1_REG2,      // 7
    MEM_REG2_SCALE,         // 8
    MEM_IMM_REG2_SCALE,     // 9
    MEM_REG1_REG2_SCALE,    // 10
    MEM_IMM_REG1_REG2_SCALE // 11
} od_type_t;
...
typedef struct OPERAND_STRUCT
{
    ...
    uint64_t scale; // scale number to register 2
...
static uint64_t decode_operand(od_t *od)
{
    ...
    else
    {
        ...
        else if (od->type == MEM_REG2_SCALE)
        {
            vaddr = (*((uint64_t *)od->reg2)) * od->scale;
        }
        else if (od->type == MEM_IMM_REG2_SCALE)
        {
            vaddr = od->imm + (*((uint64_t *)od->reg2)) * od->scale;
        }
        else if (od->type == MEM_REG1_REG2_SCALE)
        {
            vaddr = (*((uint64_t *)od->reg1)) + (*((uint64_t *)od->reg2)) * od->scale;
        }
        else if (od->type == MEM_IMM_REG1_REG2_SCALE)
        {
...
static void parse_instruction(const char *str, inst_t *inst, core_t *cr)
{
    ...
    for (int i = 0; i < len; ++i)
    {
        ...
        else if (state == 3 && c == ',' && (count_parenttheses == 0 || count_parenttheses == 2))
        {
            state = 4;
            continue;
        }
...
{
    if ((UNIT_TEST_SET & UNIT_TEST_PARSE_INSTRUCTION) == 0)
    {
        return;
    }
    ACTIVE_CORE = 0x0;
 ...
 * @param cr active core processer
 */
static void parse_operand(const char *str, od_t *od, core_t *cr)
{
    od->type = EMPTY;
    ...
    else
    {
        ...
        int ca = 0; // counter of bracket
        int cb = 0; // counter of comma
        ...
        else if (cb == 2)
        {
            if (reg1_len > 0)
            {
                if (imm_len > 0)
                {
                    od->type = MEM_IMM_REG1_REG2_SCALE;
                    return;
                }
                else
                {
                    od->type = MEM_REG1_REG2_SCALE;
                    return;
                }
            }
            else
            {
                if (imm_len > 0)
                {
                    od->type = MEM_IMM_REG2_SCALE;
                    return;
                }
                else
                {
                    od->type = MEM_REG2_SCALE;
                    return;
                }
            }
        }
...
{
    if ((UNIT_TEST_SET & UNIT_TEST_PARSE_OPERAND) == 0)
    {
        return;
    }
    ACTIVE_CORE = 0x0;
...
static void add_handler(od_t *src_od, od_t *dst_od, core_t *cr)
{
    ...
    if (src_od->type == REG && dst_od->type == REG)
    {
        ...
        cr->flags.OF = (src_sign == dst_sign) ^ val_sign; // signed overflow
...
void instruction_cycle(core_t *cr)
{
    ...
    readints_dram(va2pa(cr->rip, cr), inst_str, cr);
